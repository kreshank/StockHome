...
let daysum_tests =
  ...
  >::: [
         ...
         ( "52wk" >:: fun _ ->
           assert_equal (Some 124.17, Some 198.23) (DaySum.week_range_52w aapl)
         );
         ( "avg vol" >:: fun _ ->
           assert_equal (Some 57268612) (DaySum.avg_day_vol aapl) );
         ( "1y targ" >:: fun _ ->
           assert_equal (Some 198.01) (DaySum.targ_est_1y aapl) );
         ("beta" >:: fun _ -> assert_equal (Some 1.31) (DaySum.beta_5y_mly aapl));
         ( "bid" >:: fun _ ->
           assert_equal (Some 193.44, Some 900) (DaySum.bid aapl) );
         ( "day range" >:: fun _ ->
           assert_equal (Some 190.21, Some 194.40) (DaySum.day_range aapl) );
         ("eps" >:: fun _ -> assert_equal (Some 6.14) (DaySum.eps_ttm aapl));
         ( "earnings" >:: fun _ ->
           assert_equal
             (Some (1, 31, 2024), Some (2, 05, 2024))
             (DaySum.earnings_date aapl) );
         ( "ex date" >:: fun _ ->
           assert_equal (Some (11, 10, 2023)) (DaySum.ex_date aapl) );
         ( "forward & yield" >:: fun _ ->
           assert_equal (Some 0.96, Some 0.51) (DaySum.forward_div_yield aapl)
         );
         ( "market cap" >:: fun _ ->
           assert_equal 3008000000000. (DaySum.market_cap aapl) );
         ("open" >:: fun _ -> assert_equal 190.21 (DaySum.open_price aapl));
         ( "pe ratio" >:: fun _ ->
           assert_equal (Some 31.5) (DaySum.pe_ratio_ttm aapl) );
